...
class CartPole3DJiminyEnv(BaseJiminyEnv):
    """Implementation of a Gym environment for the Cartpole which is using
    Jiminy Engine to perform physics computations and Meshcat for rendering.

    It is a specialization of CartPoleJiminyEnv. The Cartpole is a pole attached
    by an un-actuated joint to a cart. The goal is to prevent the pendulum from
    falling over by increasing and reducing the cart's velocity.
    ...
    **OBSERVATION:**
    ...
    2	Pole Angle     x        -24 deg      24 deg
    2	Pole Angle     y        -24 deg      24 deg
    1	Cart Velocity  x        -Inf         Inf
    1	Cart Velocity  y        -Inf         Inf
    3	Pole Velocity At Tip x   -Inf         Inf
    3	Pole Velocity At Tip y   -Inf         Inf
    ...
    **SOLVED REQUIREMENTS:**
    ...
    def __init__(self, continuous: bool = False, debug: bool = False) -> None:
        ...
        data_dir = resource_filename(
            "gym_jiminy.envs", "data/toys_models/cartpole")
        urdf_path = os.path.join(data_dir, "cartpole_modified.urdf")
        # urdf_path = os.path.join(data_dir, "cartpole.urdf")
        ...
        # Add motors and sensors
        motor_joint_name = "slider_to_cart"
        motor_joint_name_second_axis = "slider_to_cart_second_axis"
        encoder_sensors_descr = {
            "slider": "slider_to_cart",
            "slider2": "slider_to_cart_second_axis",
            "pole": "cart_to_pole",
            "pole2": "cart_to_pole_second_axis",
        }
        motor = jiminy.SimpleMotor(motor_joint_name)
        robot.attach_motor(motor)
        motor.initialize(motor_joint_name)

        motor2 = jiminy.SimpleMotor(motor_joint_name_second_axis)
        robot.attach_motor(motor2)
        motor2.initialize(motor_joint_name_second_axis)
    ...
    def _refresh_observation_space(self) -> None:
        ...
        """
        # Compute observation bounds
        high = np.array([2.0 * X_THRESHOLD, 2.0 * X_THRESHOLD,
                         2.0 * THETA_THRESHOLD, 2.0 * THETA_THRESHOLD,
                         *self.robot.velocity_limit])
    ...
    def _sample_state(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        qpos = sample(scale=np.array([
            X_RANDOM_MAX, X_RANDOM_MAX, THETA_RANDOM_MAX, THETA_RANDOM_MAX]), rg=self.rg)
        qvel = sample(scale=np.array([
            DX_RANDOM_MAX, DX_RANDOM_MAX, DTHETA_RANDOM_MAX, DTHETA_RANDOM_MAX]), rg=self.rg)
        return qpos, qvel
    ...
    def is_done(self) -> bool:
        """ TODO: Write documentation.
        """
        x, y, theta, *_ = self._observation
        return (abs(x) > X_THRESHOLD) or (abs(y) > X_THRESHOLD) or (abs(theta) > THETA_THRESHOLD)
